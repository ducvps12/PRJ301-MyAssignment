  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .avatar{width:120px;height:120px;border-radius:999px;background:linear-gradient(135deg,var(--bg2),var(--bg));
      border:1px solid var(--bd);display:flex;align-items:center;justify-content:center;font-weight:700}
    .big{font-size:36px}
    ...
    input,select{width:100%;padding:10px 12px;border:1px solid var(--bd);border-radius:12px;background:transparent;
    ...
    .readonly input, .readonly select{background:color-mix(in oklab, var(--bg) 80%, transparent)}
  </style>
</head>
<body>
  ...
  <main class="wrap">
      ...
      <div class="info">
        <form method="post" action="${pageContext.request.contextPath}/profile">
          <div class="full">
            ...
            <input type="text" name="fullName" value="${u.fullName}" ${!canEdit ? 'readonly' : 'required'} />
          </div>

          <div>
            <label>Phòng ban</label>
            <%-- Cho nhập tự do, đồng thời gợi ý IT/QA/SALE qua datalist --%>
            <input list="deptList" name="department" value="${u.department}" ${!canEdit ? 'readonly' : ''}/>
    ...
    <div class="note muted" style="margin:18px 4px">
      Lưu ý: Trang này khớp với bảng <code>dbo.Users</code> (các cột: <code>username</code>, <code>full_name</code>, <code>role</code>, <code>department</code>, <code>status</code>, <code>created_at</code>).
    </div>
  </main>
