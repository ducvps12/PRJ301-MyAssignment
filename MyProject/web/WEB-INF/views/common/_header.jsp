...
<div class="left">
  <a class="brand" href="${cp}/" aria-label="Trang chủ">
    <img
      src="https://i.imgur.com/tumQO30.png"
      alt="LeaveMgmt"
      width="120" height="32"
  ...
  <nav id="mainnav" class="nav" role="navigation" aria-label="Chính">
    ...
    <a href="${cp}/request/list"
       class="${fn:startsWith(_uri, cp.concat('/request/list')) ? 'active' : ''}"
    ...
    <!-- Division Dashboard: chỉ DIV_LEADER -->
    <c:if test="${isDivLead}">
      <a href="${cp}/admin/div"
         class="${fn:startsWith(_uri, cp.concat('/admin/div')) ? 'active' : ''}"
    ...
    <c:if test="${isAdmin}">
      <a href="${cp}/admin"
         class="${fn:startsWith(_uri, cp.concat('/admin')) && !fn:startsWith(_uri, cp.concat('/admin/div')) ? 'active' : ''}"
...
<script>
  (function () {
    ...
    // Chỉ bật overlay khi điều hướng thật
