...
<head>
  ...
  <style>
    ...
    .card{max-width:720px;margin:auto;padding:20px;border:1px solid #e5e7eb;border-radius:12px}
  ...
  <div class="card">
    ...
    <c:if test="${not empty requestScope.request}">
      <div class="info">
        ...
        <p><strong>Tiêu đề:</strong> ${fn:escapeXml(request.title)}</p>
        <p><strong>Người tạo:</strong> ${request.requester.fullName}</p>
        ...
        <p><strong>Loại:</strong> ${request.leaveType}</p>
        <p><strong>Lý do:</strong> ${fn:escapeXml(request.reason)}</p>
        <p><strong>Trạng thái hiện tại:</strong> <b>${request.status}</b></p>
      </div>
      ...
      <form method="post" action="${pageContext.request.contextPath}/request/approve">
        ...
        <div class="row">
          ...
          <textarea name="note" placeholder="Ghi chú (tuỳ chọn)">${param.note}</textarea>
        </div>

        <div class="row" style="grid-template-columns:1fr 1fr;gap:10px;margin-top:16px">
          <button type="submit" name="ok" value="1" class="btn btn-approve">✔ Duyệt</button>
          <button type="submit" name="ok" value="0" class="btn btn-reject">✖ Từ chối</button>
        </div>
      </form>
