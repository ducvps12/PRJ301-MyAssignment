...
<style>
  ...
  .card{background:var(--card,#fff);border:1px solid var(--bd,#e5e7eb);border-radius:16px;padding:14px}
  ...
  .quick a{display:inline-flex;align-items:center;gap:8px;padding:10px 12px;border:1px solid var(--bd);border-radius:12px;text-decoration:none}
...
<main class="with-sb">
  <div class="wrap">
    ...
    <div class="panel" style="display:flex;align-items:center;gap:12px">
      <div class="avatar" style="width:44px;height:44px;border-radius:50%;background:#dbeafe;display:grid;place-items:center;font-weight:900;color:#1e3a8a">
      ...
      <div style="margin-left:auto" class="quick">
        <a href="${cp}/request/create">＋ Tạo đơn</a>
    ...
    <div class="cards" aria-label="Chỉ số nhanh">
      ...
      <div class="card"><h4>Ước tính NET (tháng)</h4><div class="metric"><fmt:formatNumber value="${empty requestScope.kpi.net ? 0 : requestScope.kpi.net}" type="currency"/></div></div>
    </div>
    ...
    <div class="panel" style="margin:14px 0">
      <div class="tiles">
        ...
        <a class="tile" href="${cp}/payroll"><div class="ic">💸</div><div><b>Lương thưởng</b><div style="color:var(--muted);font-size:12px">Tổng hợp</div></div></a>
        ...
        <a class="tile" href="${cp}/admin"><div class="ic">🛠</div><div><b>Admin</b><div style="color:var(--muted);font-size:12px">Dashboard</div></div></a>
      </div>
    </div>

    <!-- 2 cột: Hoạt động gần đây + Widget chấm công -->
    <div class="grid">
      <div class="panel">
      ...
      <div class="panel">
        ...
        <form method="post" action="${cp}/attendance/clock" style="display:flex;gap:8px;flex-wrap:wrap">
          ...
          <button class="btn-primary" name="action" value="in">Check-in</button>
          <button class="tb-link" style="border:1px solid var(--bd);border-radius:10px" name="action" value="out">Check-out</button>
