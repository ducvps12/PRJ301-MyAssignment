...
<head>
  ...
  <style>
    ...
    .audit .filters input,.audit .filters select{padding:.45rem .6rem;border:1px solid var(--line,#e5e7eb);border-radius:.6rem}
    .audit .table{margin-top:.75rem;background:var(--card,#fff);border-radius:.8rem;overflow:hidden;box-shadow:var(--shadow,0 1px 2px rgba(0,0,0,.05))}
    .audit table{width:100%;border-collapse:collapse}
    .audit th,.audit td{padding:.6rem .8rem;border-bottom:1px solid var(--line,#eee);font-size:14px}
    .audit th{background:var(--subtle,#f8fafc);text-align:left}
    .chip{display:inline-block;padding:.2rem .5rem;border-radius:999px;font-size:12px;border:1px solid var(--line,#e5e7eb)}
    ...
    .pagination a{padding:.35rem .6rem;border:1px solid var(--line,#e5e7eb);border-radius:.5rem;text-decoration:none}
...
<div class="container audit">
  ...
  <form class="filters" method="get" action="">
    ...
    <div style="flex:1 1 240px">
      ...
      <input name="q" placeholder="note, IP, user agent..." value="${fn:escapeXml(param.q)}" style="width:100%"/>
    </div>
    <div>
      <label>Size</label><br>
