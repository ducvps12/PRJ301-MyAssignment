...
<div class="main-body user-form-page">
  <div class="container-xl">
    ...
    <div class="uf-head">
      <div class="uf-left">
        ...
        <div class="uf-sub">
          <c:choose>
            <c:when test="${isEdit}">
              Cập nhật thông tin tài khoản. Username không thể thay đổi.
            </c:when>
            <c:otherwise>
              Nhập thông tin nhân sự, hệ thống sẽ gợi ý username + phân quyền nhanh.
            </c:otherwise>
          </c:choose>
        </div>
        <div class="uf-steps">
      ...
      <div class="uf-right">
        <div class="avatar-box" title="Ảnh đại diện tạm">
          ...
          <span class="avatar-status <c:out value='${f_status eq "INACTIVE" ? "offline" : "online"}'/>"></span>
    ...
    <div class="uf-quick-row">
      ...
      <div class="qa-hint">
        tip: click 1 nút để tự set <b>Role + Phòng ban</b>.
      </div>
    </div>
          ...
          <div class="uf-block">
            ...
            <div class="uf-field-group">
              ...
              <div class="uf-field">
                <label>
                  Mật khẩu
                  <c:if test="${!isEdit}"><span class="req">*</span></c:if>
                </label>
                <div class="pw-wrap">
                  <input type="password"
                         name="password"
            ...
            <details class="uf-advanced">
              ...
              <div class="uf-adv-hint">
                Phần này chỉ lưu để quản trị xem, nhân viên không thấy.
              </div>
            </details>
...
<script>
(function(){
  const isEdit = '${isEdit}' === 'true' || '${isEdit}' === 'true';
  ...
  if (fullName && username && !isEdit) {
    ...
    username.addEventListener('input', function(){ username.dataset.touched = '1'; });
  }
  ...
  if (emailInput){
    emailInput.addEventListener('blur', function(){
      ...
      if (v && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v)){
        ...
        if (hint) hint.textContent = 'Email không hợp lệ';
      } else {
  ...
  if (phoneInput){
    phoneInput.addEventListener('blur', function(){
      ...
      if (v && !/^[0-9]{9,11}$/.test(v)){
        this.setCustomValidity('Số điện thoại phải 9-11 chữ số');
        if (hint) hint.textContent = 'Số điện thoại phải 9-11 chữ số';
      } else {
        this.setCustomValidity('');
        if (hint) hint.textContent = '';
      }
  ...
  if (roleSelect && deptSelect){
    roleSelect.addEventListener('change', function(){
      const v = this.value;
      if (!deptSelect.value){           // chỉ gợi ý khi chưa chọn
  ...
  /* ========== LOCALSTORAGE DRAFT (mode create) ========== */
  ...
  window.addEventListener('keydown', function(e){
    ...
    if (e.key === 'Escape'){
      const back = document.getElementById('btnCancel');
      if (back){
...
.user-form-page{
  background: radial-gradient(circle at top, #e0e7ff, #eef2f7 40%, #ffffff 70%);
...
@media (max-width: 992px){
  ...
  .uf-quick-row{flex-wrap:wrap;}
  .uf-head{flex-wrap:wrap;}
  .uf-footer{flex-wrap:wrap;}
}
@media (max-width:576px){
  .uf-field-group{grid-template-columns:1fr;}
  .uf-card{padding:1.1rem .9rem .8rem;}
  .qa-btn{width:100%;justify-content:center;}
  .uf-head{align-items:flex-start;}
}
</style>
