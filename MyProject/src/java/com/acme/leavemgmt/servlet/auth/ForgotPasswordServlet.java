...
public class ForgotPasswordServlet extends HttpServlet {
    ...
    private void handleRequestOtp(HttpServletRequest req, HttpServletResponse resp)
        ...
        try (UserDAO userDAO = new UserDAO(DBConnection.getConnection())) {
            ...
            req.setAttribute("otpFocus", "1"); // focus ô OTP
            doGet(req, resp);
        } catch (Exception ex) {
    ...
    private void handleResetPassword(HttpServletRequest req, HttpServletResponse resp)
        ...
        try (UserDAO userDAO = new UserDAO(DBConnection.getConnection())) {
            String hash = Passwords.hash(newPwd); // PBKDF2/SHA256 tuỳ util
            ...
            ResponseUtils.redirectWithMessage(
                    req, resp, req.getContextPath() + "/login",
